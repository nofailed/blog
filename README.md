...
# Set the source code directory paths
linux_src=/usr/src/linux-$(uname -r)
android_src=/path/to/android/source
android_base_src=/path/to/android/base/source

# Generate tags file for Linux kernel and Android firmware
ctags -R --c-kinds=+p --c++-kinds=+p --fields=+iaS --extra=+q \
      $linux_src $android_src $android_base_src \
      --languages=Java

# Generate cscope files for Linux kernel and Android firmware
find $linux_src $android_src $android_base_src \
  -type f \( -name "*.c" -o -name "*.cpp" -o -name "*.h" \
  -o -name "*.hpp" -o -name "*.cc" -o -name "*.cxx" -o -name "*.java" \) \
  -print > cscope.files
